...
| 1. Решётка 16 эл. на ПЛИС (приём + передача) | Живые сценарии 1–9; железо и прошивка — [docs/hardware](hardware/architecture.md) | 🔧 схемы и RTL готовы, закупка |
| 2. Приём: камера и адаптивка | Акустическая камера, «направленное ухо», MVDR (7–9) | ⏳ «направленное ухо» уже в RTL |
...
## 7. Решения по формату (приняты)

* Аудитория: школьная конференция и музей кафедры → автономный экспонат с пультом на планшете.
* Приём и передача, акцент на передаче → 16 «ППМ» (динамик + микрофон).
* Бюджет ≤ 1000 $, только МК/ПЛИС → ULX3S (ECP5 + ESP32), ≈ 820 $ с резервом.

## 8. Открытые вопросы

* Помещение музея: размеры, отделка стен (реверберация), где встанет экспонат.
* Нужен ли большой экран (HDMI от ПЛИС) для «акустической камеры», или хватит планшета.
